...
echo [2/7] Starting Document API Server (wsgi_doc_api.py) in aihubant environment...
...
echo [3/7] Starting Document Job Queue (app_doc_job_q.py) in aihubant environment...
...
echo [4/7] Starting JSS Main Application (app_jss_main.py) in jss environment...
...
echo [5/7] Starting Vector API Server (wsgi_vector_api.py) in aihubvector2 environment...
...
echo [6/7] Starting Agent API Server (wsgi_agent_api.py) in aihub2 environment...
start "AIHub Agent API" /D "%PROJECT_PATH%" cmd /k "color 0F && title AIHub Agent API && "%CONDA_PATH%\Scripts\activate.bat" && conda activate aihub2 && python wsgi_agent_api.py"

timeout /t 3 /nobreak >nul

echo [7/7] Starting Knowledge API Server (wsgi_knowledge_api.py) in aihub2 environment...
start "AIHub Knowledge API" /D "%PROJECT_PATH%" cmd /k "color 0F && title AIHub Knowledge API && "%CONDA_PATH%\Scripts\activate.bat" && conda activate aihub2 && python wsgi_knowledge_api.py"

timeout /t 3 /nobreak >nul

echo [1/7] Starting Main Application (wsgi.py) in aihub2 environment...
start "AIHub Main App" /D "%PROJECT_PATH%" cmd /k "color 0A && title AIHub Main App && "%CONDA_PATH%\Scripts\activate.bat" && conda activate aihub2 && python wsgi.py"
